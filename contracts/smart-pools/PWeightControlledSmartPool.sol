...
contract PWeightControlledSmartPool is PCappedSmartPool {
    ...
    function updateWeightsGradually(uint256[] calldata _newWeights, uint256 _startBlock, uint256 _endBlock)
        external
        noReentry
        onlyController
    {
        ...
        if(block.number > _startBlock){ // This means the weight update should start ASAP
        ...
        require(_endBlock > _startBlock, "PWeightControlledSmartPool.updateWeightsGradually: End block must be after start block");
        for (uint i = 0; i < tokens.length; i++) {
        ...
        noReentry
    {
        ...
        uint blockPeriod = ws.endBlock.bsub(ws.startBlock);
        uint weightDelta;
        uint newWeight;
        address[] memory tokens = s.bPool.getCurrentTokens();
        for (uint i = 0; i < tokens.length; i++) {
            if (ws.startWeights[i] >= ws.newWeights[i]) {
                ...
                newWeight = ws.startWeights[i].bsub(
                                (minBetweenEndBlockAndThisBlock.bsub(ws.startBlock)).bmul(
                                    weightDelta.bdiv(blockPeriod)
                                )
                            );
            } else {
