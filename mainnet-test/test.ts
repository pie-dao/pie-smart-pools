...
describe("MAINNET TEST", function() {
    ...
    it("Setting public swap setter from non controller should fail", async() => {
        ...
        expect(await transactionFails(poolAS.setPublicSwapSetter(account2, { gasLimit: 2000000 }))).to.be.true;
    ...
    it("Unbinding a token from a non tokenBinder should fail", async() => {
        expect(await transactionFails(poolAS.unbind(tokens[0].address, { gasLimit: 2000000 }))).to.be.true;
    ...
    it("Binding a token from a non tokenBinder should fail", async() => {
        ...
        expect(await transactionFails(poolAS.bind(mockToken.address, constants.WeiPerEther, constants.WeiPerEther, { gasLimit: 2000000 }))).to.be.true;
    ...
    it("Rebinding a token from a non tokenBinder should fail", async() => {
        ...
        expect(await transactionFails(poolAS.rebind(tokens[0].address, balanceBefore.div(2), constants.WeiPerEther, { gasLimit: 2000000 }))).to.be.true;
